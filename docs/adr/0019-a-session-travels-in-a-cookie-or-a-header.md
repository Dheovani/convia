...
- **The interface never holds the token, in either form.** In a browser the cookie is `HttpOnly`; in the application the header is attached by the Go process on the interface's behalf, after its request has left the webview and before it reaches Convia. The six routes that would answer with a session are refused there and reached through the application's own methods instead, which return who is signed in and not what signed them in. `M35-007` built that; `docs/interface.md` describes it.
- **A webview says it is a browser, and has to be told not to.** The origin, the site-context and the cookies a webview sends would make Convia answer with a cookie this process cannot keep. They are removed on the way out, which means the distinction this ADR draws is enforced by the client as well as read by the server.
