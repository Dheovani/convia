...
- **Installations on a private network can invite each other only when their operator allows it**, with `CONVIA_PEERS_ALLOW_PRIVATE_ADDRESSES`. This first followed development mode, as webhook delivery does, but development is the default, and anybody who can register on an installation left in it could use links to map the network behind it. The setting is off in every environment; see `docs/peers.md`.
