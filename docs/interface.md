...
**While the stream is not open, the interface asks as it did before there was one** — the sidebar every fifteen seconds, an open room every five. Being told is an improvement on asking, never a replacement for being able to ask. When the stream opens again, the sidebar is read, the open room reads forward for what is new, and then its newest window again for what was edited or withdrawn.

It reconnects after a second, doubling to half a minute, so that every tab of every person does not reconnect at once into a Convia that is still down. **It reconnects with the cursor of the last event it received**, so what happened while it was away arrives before anything new; a cursor older than Convia keeps (`4002`) is dropped, and the next connection starts afresh.
