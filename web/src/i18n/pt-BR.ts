...
export const ptBR: Words = {
  ...
  conversation: {
    removedByOwner: 'Removida pelo dono da sala ou por um moderador.',
  ...
  people: {
    ...
    nameModerator: 'Tornar moderador',
    nameModeratorNamed: (name) => `Tornar ${name} moderador`,
    unnameModerator: 'Tirar moderação',
    unnameModeratorNamed: (name) => `Tirar a moderação de ${name}`,
    roleFailed: (name) => `Não foi possível mudar o que ${name} pode fazer.`,
    handOver: 'Tornar dono',
    handOverNamed: (name) => `Tornar ${name} dono`,
    confirmHandOver: (name, room) =>
      `Tornar ${name} dono de ${room}? Você continua nela como membro, e só ${name} pode devolvê-la.`,
    handOverConfirm: 'Tornar dono',
    handingOver: 'Transferindo…',
    keepOwning: 'Manter',
    handOverFailed: (name) => `Não foi possível tornar ${name} dono.`,
