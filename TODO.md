...
- [ ] **M18-018:** Give a person something to subscribe to. The interface **polls** — the sidebar every fifteen seconds, an open room every five — because Convia's event stream is on the surface an application reaches with its key and a session has no way to subscribe to its own rooms. This is a gap the interface exposed rather than one it created, and the shape of the answer is not obvious: a person's stream is authorized per room rather than per tenant, so it is not the existing handler with a different verifier.
...
- [ ] **M18-020:** Announce changes of membership. There is no `member.*` event, so a person added to a room finds out when the sidebar next asks, and an application learns of a person adding or leaving only from the audit log. It is recorded rather than built with `M18-003` because nothing could consume it yet: a person has no stream to receive it on until `M18-018`.
