...
- **Current milestone:** M15 — Webhooks for External Applications. **Complete except manual redelivery** (`M15-010`), which is deferred because it needs the operator-facing authority and audit trail that M20 defines. M14 is complete except the resume cursor (`M14-006`), which needs a durable event log, and the metrics (`M14-013`), which need a metrics pipeline. M14 also supplied what `M12-010` was waiting for: a Convia-owned event to translate a provider event into. M13 remains in progress: what is left there is the rate limit (`M13-008`) and the SDK-facing connection guide (`M13-010`). M13 also closed `M12-005` and `M12-006`. M12's remaining items need webhook verification (`M12-009`, `M12-010`) or measured failure modes (`M12-013`, `M12-014`). M11, M10, M09 and M08 are complete. M07 is done except `M07-009`, which needs the origin model of the standalone web application and is blocked on M18 rather than outstanding. Phase 0 and M06 are complete.
...
- **Next implementation milestone:** M16 — Redis and Distributed Ephemeral State, for which `M14-014` and `M16-001` have now recorded the first concrete use case
