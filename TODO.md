...
- **Deferred for one reason in several places:** metrics (`M14-013`, `M16-010`, `M17-011`) wait for `M22`; a general per-tenant rate limit is `M13-008` and `M23-013`; packaging and the tests that need a package (`M35-011`, `M35-012`) wait for a product worth handing to somebody.
...
- [x] **M07-009:** Define first-party standalone UI session behavior separately from external API credentials. **Closed by `M18-002`**: a session is a credential family of its own and carries no authority over a tenant. It lived only in a cookie until `M35-003` let a client that is not a browser hold one; see [ADR 0019](docs/adr/0019-a-session-travels-in-a-cookie-or-a-header.md). It was deferred to M18 rather than done here: browser sessions needed the origin and cookie model of the standalone web application, which did not exist yet. What M07 settled is the *server-to-server* separation — operator credentials are their own family, so an application key can never administer Convia — which is a different question from how a person's browser holds a session.
...
- [ ] **M33-001:** Tell a visitor's installation what happens in a room elsewhere as it happens, so the page stops reading it on a five-second timer and can show an unread count. Decide whether the home pushes to the visitor's installation or the visitor's installation holds a signed stream open, and what either costs a home with many visitors. **The decision is made and the stream is built: the visitor's installation pulls.** The other way round needs an identity an installation does not have — a home cannot prove itself to anybody, and it does not know where a visitor's installation is — so it would mean inventing a key per installation, a way to trust it, and a delivery queue, which is the opposite of what [ADR 0012](docs/adr/0012-a-room-lives-on-one-installation-and-visitors-sign.md) settled. Pulling needs none of it: the handshake is signed with the person's own key like every other request, and the home serves `GET /v1/peer/events` with the same handler it serves `/v1/me/events` with. **The cost is one connection per visitor who is connected**, and the key is opened from their session, so it cannot outlive their being signed in. The timer is gone. **What is left is the unread count**, which is a different question: the count lives at the home, and nothing asks it for one yet — `GET /v1/me/remote-rooms` reads only the pointers kept here.
...
- [x] **M33-005:** Configure an installation's public address rather than deriving an invitation link from the browser's `Origin`. Behind a reverse proxy, or when an administrator opens the page at an internal address, the `Origin` is not where other installations reach this one. `CONVIA_PUBLIC_ADDRESS` is that address, checked once at startup so a value nobody can make a link out of stops the process instead of producing links that name the wrong place. It **outranks everything a request can offer**, because every address a request offers is the address *that* person reached Convia at. Unset, the old guess stands, and for one machine on one network it is right. An installation now answers to **both** of its addresses when deciding whether a link leads back to it, so configuring one does not break the invitation somebody is already holding, or the one made by whoever is sitting at the machine. It deliberately does **not** touch the authority a signed request is checked against: that is still the `Host`, a proxy must still pass it through, and whether the configured address should stand there too belongs with `M33-003`.
...
**Priority:** P1
**Status:** In progress. Every item is complete except `M35-011` and `M35-012`, which are packaging and wait for a product worth packaging.
...
- [ ] **M35-011:** Build a Windows installer, decide what signing costs and whether the first version is signed, and decide how the app updates itself. An unsigned installer is a warning every person who installs it has to walk past. **Deferred**, by the product owner: the system is not ready for anybody outside it to install, so a certificate, an installer and an update channel would be spent delivering something still being built. Nothing depends on it — the application is built and run from source with `./scripts/app.sh` — and it returns when there is a version worth handing to somebody. The one part with a lead time is the certificate, which is why it is named here rather than left to be discovered on the day.
- [ ] **M35-012:** Decide what tests the app needs: the interface keeps its own suite, and the journeys `M18-019` drives in a browser have to either keep serving that purpose or be replaced by something that drives the packaged app. **Deferred with `M35-011`**, because half of it tests a package that does not exist. The other half does not need one and is the part worth keeping in sight: the five Playwright journeys run a real browser, the browser is no longer the product, and what they are evidence of has therefore changed without anybody deciding it.
- [x] **M35-013:** Put the written record right: `docs/interface.md` describes the app, [ADR 0009](docs/adr/0009-convia-serves-its-own-interface-from-its-own-origin.md) is superseded where it argued for serving a page to browsers, and the README stops presenting the page as the product. `docs/interface.md` and ADR 0009 were done as the work happened. The README was the half that was left, and it was worse than stale: it opened by calling the interface something Convia *intends* to provide, led with the page compiled into the binary and served from the API's origin, and said a session **lives only in a cookie**, which `M35-003` had made untrue. It now leads with the application, puts the page where it belongs — how the interface is worked on — and names both halves of ADR 0019. `Run` offers the application before the development server, `Build` has the second binary with the Wails tags that are the difference between a window and a dialog explaining there is none, and `Prerequisites` says a machine needs Windows and WebView2 for that half and nothing for the rest.
