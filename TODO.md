...
- **Current milestone:** M11 — Internal Media Boundary. **Complete.** M10 is complete except its invitation slice, which is deferred to the join sessions of M13 for the reason recorded in `M10-004`. M09 and M08 are complete. M07 is done except `M07-009`, which needs the origin model of the standalone web application and is blocked on M18 rather than outstanding. Phase 0 and M06 are complete.
...
- **Next implementation milestone:** M12 — LiveKit Adapter, which is the first implementation behind the boundary M11 defined
...
- **Current media capability:** none, deliberately and coherently. `internal/media` is the boundary between the control plane and whatever transports audio and video, and `media.Absent` is what Convia ships with: every session realizes as nothing, so rooms, calls, and participants all work and nobody can connect. A call whose session cannot be realized is ended rather than left holding its room, and a test asserts media infrastructure never leaves `internal/media`. See [ADR 0001](docs/adr/0001-control-plane-media-plane-boundary.md)
