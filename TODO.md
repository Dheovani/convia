...
- **Current milestone:** M18 — Standalone Web Application. A person registers, signs in, opens and moderates rooms, holds conversations that update as they happen, and shares rooms with people on other installations. What remains is calls (`M18-004` to `M18-008`) and the interface's own gaps (`M18-009` to `M18-014`, `M18-019`, `M18-021`, `M18-026` to `M18-031`).
- **Next implementation milestone:** `M18-004` — calls in Convia's own product, with the shape the product owner decided recorded on the item.
...
- **Deferred for one reason in several places:** metrics (`M14-013`, `M16-010`, `M17-011`) wait for `M22`; a resume cursor for streams (`M14-006`) waits for a durable event log; the transactional outbox is `M15-016`; provider webhooks are `M12-009` and `M12-010`; a general per-tenant rate limit is `M13-008` and `M23-013`.
...
- **Calls and media.** Calls, participants with roles and capacity, invitations for users and for guests, and short-lived media credentials a real LiveKit accepts, behind a boundary that keeps the provider out of the domain and the contract. See [`docs/calls.md`](docs/calls.md), [`docs/participants.md`](docs/participants.md), [`docs/invitations.md`](docs/invitations.md) and [`docs/media.md`](docs/media.md).
...
- **Convia's own product.** Registration with nothing to configure, handles, and an interface served from the API's own origin. See [ADR 0009](docs/adr/0009-convia-serves-its-own-interface-from-its-own-origin.md) and [ADR 0011](docs/adr/0011-an-account-is-local-and-its-identifier-is-its-key.md).
...
- [x] **M09-004:** Define actor and reason fields for transitions. The actor is `application` or `operator` and is taken from the verified credential, never from a request field: a body that could name the actor would let an application record an operator's name against its own decision. The reason is optional free text, because the reasons a conversation ends are not Convia's to enumerate. A signed-in person starting or ending a call will be a third actor, `person`, added by `M18-004`.
...
- [ ] **M18-004:** Build call start, join, leave, and end flows. The product owner decided the shape:
  - any member of a room starts a call in it, and a call ends when its last participant leaves; nobody ends it for everybody;
  - a person starting or ending a call is recorded as a third actor, `person`, beside `application` and `operator`, without saying which person;
  - the first delivery carries audio and video;
  - the room's owner joins its calls as `moderator` and everybody else as `member` (`M18-025`);
  - visitors from other installations join calls later, in `M33-002`.

  It starts with the session surface for calls, as rooms and messages did, and the page's content policy has to allow the media plane's address, which `web.go` already says.
