...
- **Current milestone:** M16 — Redis and Distributed Ephemeral State. **Complete except metrics** (`M16-010`), deferred for the same reason as M14's: there is no metrics pipeline yet. M16 also closes `M14-014`'s remaining half — a deployment can now run more than one instance without narrowing the event stream. M15 is complete except manual redelivery (`M15-010`), which needs the operator-facing authority and audit trail that M20 defines. M14 is complete except the resume cursor (`M14-006`), which needs a durable event log, and the metrics (`M14-013`), which need a metrics pipeline. M14 also supplied what `M12-010` was waiting for: a Convia-owned event to translate a provider event into. M13 remains in progress: what is left there is the rate limit (`M13-008`) and the SDK-facing connection guide (`M13-010`). M13 also closed `M12-005` and `M12-006`. M12's remaining items need webhook verification (`M12-009`, `M12-010`) or measured failure modes (`M12-013`, `M12-014`). M11, M10, M09 and M08 are complete. M07 is done except `M07-009`, which needs the origin model of the standalone web application and is blocked on M18 rather than outstanding. Phase 0 and M06 are complete.
...
- **Next implementation milestone:** M17 — Presence, which now has the ephemeral store it depends on
...
- **Current scaling capability:** a deployment can run more than one instance without the event stream narrowing to whichever one a client happened to reach. `CONVIA_REDIS_URL` is what turns that on, and it is an operational requirement rather than a tuning option — several instances with it unset is the one configuration that is quietly wrong, so each instance says at startup which of the two it is. Redis carries **only** what instances tell each other, over publish/subscribe, which stores nothing: there are no keys to expire, nothing for an eviction policy to reclaim, and a test asserts the key count does not move while events are carried. Broadcasting cannot block or fail the request that produced the event, and an unreachable Redis narrows the stream back to one instance rather than failing anything — readiness deliberately ignores it. See [`docs/events.md`](docs/events.md) and [ADR 0005](docs/adr/0005-redis-for-what-instances-tell-each-other.md)
