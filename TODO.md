...
- **Current milestone:** M18 — Standalone Web Application. A person registers, signs in, opens and moderates rooms, holds conversations that update as they happen, and shares rooms with people on other installations. A person starts and joins calls in their rooms, with audio and video, and the room's owner moderates them. What remains of calls is permissions, devices, a preview and call-state feedback (`M18-005` to `M18-008`), and the interface's own gaps (`M18-009` to `M18-014`, `M18-019`, `M18-021`, `M18-026` to `M18-031`).
- **Next implementation milestone:** `M18-005` to `M18-008` — the rest of calls in Convia's own product.
...
- [ ] **M18-005:** Add microphone and camera permission handling. Since `M18-004` a refused microphone does not keep anybody out of a call and says that nobody can hear them; asking deliberately, and explaining how to allow it again, is what remains.
- [ ] **M18-006:** Add device selection and persisted preferences.
- [ ] **M18-007:** Add pre-join media preview.
- [ ] **M18-008:** Add participant roster and call-state feedback. Since `M18-004` the stage names who is in the call, marks who is muted and who is speaking, and a room's header says whether it is holding a call; what remains is the rest of a call's state as it changes — connection quality, somebody reconnecting — and saying who arrived and who left.
