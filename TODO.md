...
- **Current milestone:** M12 — LiveKit Adapter. **In progress:** the adapter is implemented and integration-tested against a real server; what remains is the work that needs a participant who can connect (`M12-005`, `M12-006`, half of `M12-008`), an internal event concept (`M12-009`, `M12-010`), or measured failure modes (`M12-013`, `M12-014`). M11 is complete. M10 is complete except its invitation slice, which is deferred to the join sessions of M13 for the reason recorded in `M10-004`. M09 and M08 are complete. M07 is done except `M07-009`, which needs the origin model of the standalone web application and is blocked on M18 rather than outstanding. Phase 0 and M06 are complete.
...
- **Next implementation milestone:** M13 — Client Session Bootstrap, which issues the credential a participant connects with and finishes the deferred half of M12
...
- **Current public contract:** [`api/openapi.yaml`](api/openapi.yaml), OpenAPI 3.0.3, covering the operational health and readiness endpoints, the authenticated tenant endpoints for users, credentials, rooms, calls, and participants, the authenticated operator endpoints for applications, operator credentials, and every tenant resource, both security schemes, and the shared error, pagination, idempotency, and correlation components
...
- **Current persistence capability:** PostgreSQL through `pgxpool`, reversible embedded migrations run by `convia migrate`, an isolated integration-test database per test, and the `applications`, `users`, `credentials`, `operator_credentials`, `rooms`, `idempotency_keys`, `calls`, and `participants` tables, with a call's media session stored beside it and never published
...
**Priority:** P0
**Status:** Complete for server-to-server credentials; `M07-009` blocked on M18
...
**Priority:** P1
**Status:** In progress - participation is complete; invitations and guests are their own slice
**Depends on:** M09
...
- [ ] **M10-004:** Define invite creation, acceptance, rejection, expiration, and revocation. Deferred to its own slice, alongside the join sessions of M13. An invitation is only authorization when the party presenting it is not the party that granted it; today the application creates one with its key and joins someone with the same key, so Convia would be checking the application's homework against itself. That is the same reasoning that kept room membership out of M08. M13's join session is presented by a client rather than by the backend that issued it, and expiry and revocation start to mean something there.
- [ ] **M10-005:** Add participant and invitation persistence. Participants done: migration `00009`, with the presence uniqueness index, the roster and status indexes, and every invariant the domain relies on encoded as a constraint, including the three shapes a row may take. Invitation persistence waits with `M10-004`.
...
- [ ] **M10-010:** Define guest participation requirements. Deferred with `M10-004`. A guest is someone with no Convia user, and the only way they get in is by presenting something Convia issued, which is the join session of M13. Admitting one on the application's word alone would be indistinguishable from the application resolving them as a user first.
- [ ] **M10-011:** Add REST endpoints and OpenAPI schemas. Participants done: nine operations across both surfaces, with `Participant`, `ParticipantPage`, `ParticipantRole`, `ParticipantStatus`, `ParticipantRemover`, and the request schemas. The contract test proves routes, security, and error codes match the implementation in both directions, and that every response body validates against the schema it claims. Invitation routes wait with `M10-004`.
...
**Exit criteria:** Participation is authorized, capacity-safe, reconnectable, auditable, and independent from LiveKit identities. **Met for participation:** every join is authorized in the control plane alone, capacity is settled by a lock rather than by hope, a reconnection returns the participant already present, every transition is audited without application-composed text, and nothing in the domain or the contract names a media provider. The invitation half of the milestone remains.
...
- [ ] **M12-005:** Map Convia participant capabilities to least-privilege provider grants. **Deferred to M13**, which is where a participant first receives a credential. The rule is already applied to Convia's own calls — a token carries the single permission its one request needs, verified by a test that compares the grant's exact key set rather than checking known fields, so a participant grant copied in later fails it.
- [ ] **M12-006:** Issue short-lived media connection credentials. **Deferred to M13.** The signing side exists and is short-lived already: Convia's own API tokens live one minute, are minted immediately before a single request, and appear nowhere but its `Authorization` header.
...
- [ ] **M12-010:** Translate provider events into Convia-owned internal events. **Deferred**, because there is no Convia-owned internal event to translate them into yet. Defining one here would fix its shape before M14 designs the envelope that has to carry it.
