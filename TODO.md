...
- **Current milestone:** M18 — Standalone Web Application, **first parcel: a person can sign in.** `M18-001` (the stack) and `M18-002` are done, and with them `M07-009`, which was deferred to this milestone precisely because browser sessions needed the origin and cookie model the standalone application defines. No frontend file exists yet, deliberately: the interface had two prerequisites that did not exist, and this is the one everything else depends on.
...
- **Next implementation milestone:** **M31-009's session surface** — `/v1/me/rooms` and the message routes under it, where the author is the session rather than a field in the request. Room membership is built and enforceable; nothing enforces it yet, because no route lets a person act as themselves. That surface is what the interface shell is waiting on, and it is the first place a session principal decides anything. **M32 — Contacts and Direct Rooms is deliberately after the interface**, not before: every part of it is a consent surface, and consent surfaces are the ones most punished by speculative design. Nothing outside Convia has ever consumed Convia — every verification so far is from the inside out, no browser has entered a call, and the media path is proved as far as the LiveKit handshake and not a step further. That is still the largest unexamined assumption in the project, and it is what `M13-010` and `M07-009` are both waiting on
...
- [x] **M18-001:** Choose the frontend stack based on team capability and long-term maintenance. **React, TypeScript, and Vite**, with Convia serving the built assets from its own origin. The stack is the boring choice on purpose — the largest hiring pool, and the TypeScript SDK `M19` delivers is what the application will consume. Same-origin is the part that mattered most and it was decided here rather than later: it is what makes the session cookie first-party, `SameSite=Lax` meaningful, and **CORS unnecessary entirely**. No frontend file exists yet; this parcel built what a login screen needs to exist at all.
...
- [ ] **M31-009:** Define per-person authorization, which is the first real use of a session principal. **Room membership is built** -- the concept `00009` declined to model for a reason M18 invalidated. It gates people rather than applications: an application's key already has full authority over its own rooms, so membership is what the *session* surface checks, where somebody acts as themselves. It carries no role and no lifecycle, and its scopes are new rather than reused, because folding it into `rooms:write` would widen every key already issued into one that can put anybody anywhere. **What remains is the session surface itself**: `/v1/me/rooms` and the message routes under it, where the author is the session rather than a field in the request. See [`docs/rooms.md`](docs/rooms.md).
