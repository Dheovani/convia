...
- **Next implementation milestone:** the guest slice that finishes M10, then M14 — Real-Time Control Events
...
**Priority:** P1
**Status:** In progress - participation and invitations are complete; guest participation is its own slice
**Depends on:** M09
...
- [ ] **M10-010:** Define guest participation requirements. **The mechanism a guest needs now exists** — an invitation is presented by its holder, which is the only way somebody with no Convia user could get in. What remains is a schema change rather than a missing idea: `participants.user_id` is `NOT NULL` and the index that makes presence unique is built on it, so admitting a person Convia has no account for means deciding what a participation without a user looks like and what, if anything, Convia should learn about them. That is its own migration and its own privacy question, not a flag on this flow.
