...
- **Current milestone:** M18 — Standalone Web Application. A person registers, signs in, opens and moderates rooms, holds conversations that update as they happen, and shares rooms with people on other installations. A person starts and joins calls in their rooms, with audio and video, and the room's owner moderates them. They get ready to join with a preview and their chosen devices, and a call says how it is going. What remains is the interface's own gaps (`M18-009` to `M18-014`, `M18-019`, `M18-021`, `M18-026` to `M18-031`).
- **Next implementation milestone:** `M18-009` to `M18-014` — responsive layouts, keyboard and screen-reader access, the edges of devices and networks, and end-to-end tests of the critical call journeys.
...
- [x] **M18-004:** Build call start, join, leave, and end flows. The product owner decided the shape:
  - any member of a room starts a call in it, **starting it puts them in it**, and a call ends when its last participant leaves; nobody ends it for everybody;
  - a person starting or ending a call is recorded as a third actor, `person`, beside `application` and `operator`, without saying which person, and **a call Convia ends without being asked is `system`**;
  - the first delivery carries audio and video, **joining with the microphone on and the camera off**;
  - the room's owner joins its calls as `moderator` and everybody else as `member` (`M18-025`), and **the moderator takes somebody out of the call, who is disconnected at once and cannot return to it**;
  - **the media server reports when somebody's connection went away**, which is `M12-009` and `M12-010`;
  - **deleting a room ends its call**, whoever deleted it, and **losing one's place in a person's room takes one out of its call**;
  - **the call is in its room**, the Calls destination lists the calls running in the person's rooms, and **a call goes on while its person reads another room**;
  - visitors from other installations join calls later, in `M33-002`.

  `POST /v1/me/rooms/{room_id}/call/join` starts or joins and answers `201` or `200`, `.../call/leave` ends the call when it empties, `GET /v1/me/calls` lists what could be joined, and the call's roster and removal sit beside them. Whether a call is empty is decided under the call's row lock, which joining takes too, and a join racing the last leave is run twenty times against a real database. The page loads the media client only on a first join, its policy names the media server exactly, and a lost connection is retried once while being taken out, the call ending and joining from another page are said. An end-to-end run with two browsers against a real LiveKit caught two things the other tests could not: the page must tell Convia it is leaving before it hangs up, or the media server's report wins and the ending is recorded as `system`; and a request the page cancelled was charged to the failed-authentication budget, which locked the page out mid-call — neither is charged nor raced now. [ADR 0014](docs/adr/0014-a-call-in-a-room-ends-when-its-people-leave.md) records all of it.
...
- [ ] **M18-011:** Handle denied permissions and unavailable devices clearly. Since `M18-005` each refusal is said by its kind and a refused permission says how to allow it again; what remains is a device that goes away in the middle of a call, which the browser reports and the page does not yet say.
- [ ] **M18-012:** Handle reconnection and degraded network states. Since `M18-004` a lost connection is retried once, and since `M18-008` a reconnection and a weak connection are said; what remains is adapting to a network that stays degraded — sending less video, or continuing with audio alone.
...
- [x] **M18-023:** Invite somebody on another installation into a room. The product owner decided the shape before anything was built:
  ...
  Known limits:
  - rooms elsewhere are read on a timer and carry no unread count (`M33-001`);
  ...
  - calls between installations are `M33-002`.
...
- [x] **M18-025:** Give a room a person opens an owner who moderates it. **This reverses the "no role" of `M18-003` and `00018`**, which were decided without the product owner. The product owner decided the rest:
  ...
  - the owner removes members, bans them, takes down what others said, and renames, closes, reopens and deletes the room, and will moderate its calls (`M18-004`);
  - a removed person can be added back by anybody, and a banned one by nobody until the owner lifts the ban, which also stops invitations and links;
  - an owner who leaves passes the room to its longest-standing member who signs in on the room's installation;
  - existing rooms get the owner that rule implies;
  - a message the owner takes down says so.

  How it holds:
  - **Ownership.** It is a column on the room whose pair with the room is a foreign key into `room_members`, checked at commit and cleared when the membership goes, so an owner is always a member.
  - **Succession.** It runs in the same transaction as every change of membership, under the room's lock.
  - **Bans.** They are checked under the same lock, and a test runs a ban and an addition concurrently.
  - **Refusals.** A member is refused an owner's act with `403`, and anybody else with `404`.

  Migration `00021`. See [ADR 0013](docs/adr/0013-a-room-a-person-opens-has-an-owner.md).

  Known limits:
  - renaming, closing and deleting announce nothing, so other members see the change on their next read;
  - there is one owner, and it cannot be handed over deliberately;
  - a visitor never owns or inherits a room.
- [ ] **M18-026:** Change the password from the interface. `PATCH /v1/me/password` exists and re-seals the account's key; the Settings destination that would hold it is still disabled. Say on the form what the registration form says: nothing can reset a password that is lost.
...
- [ ] **M18-028:** Withdraw a room invitation from the interface. `DELETE /v1/me/room-invitations/{invitation_id}` exists; the People panel forgets a link once it is shown.
- [ ] **M18-029:** Announce the changes a room's owner makes to the room itself — renamed, closed, reopened, deleted — to the streams of the people in it, and decide whether they also reach applications' streams and webhooks. Today other members see them on their next read.
- [ ] **M18-030:** Decide ownership beyond one person: handing a room over deliberately, and whether a room may have more than one moderator. `M18-025` gives a room exactly one owner and changes it only by succession.
