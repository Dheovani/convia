...
- **Next implementation milestone:** M31 — Messaging and Conversations, then the interface shell on top of it. Nothing outside Convia has ever consumed Convia: every verification so far is from the inside out, no browser has entered a call, and the media path is proved as far as the LiveKit handshake and not a step further. That is the largest unexamined assumption in the project, and it is also what `M13-010` and `M07-009` are both waiting on
...
- [ ] **M31-009:** Define per-person authorization, which is the first real use of a session principal. **Depends on a concept Convia does not have yet: room membership.** Migration `00009` records that Convia does not model it, and gives a reason M18 invalidated -- that Convia held no credentials for an application's people. Deriving membership from participation was considered and rejected: a chat-first room is one somebody joins and reads before any call happens. Membership belongs to the rooms domain rather than to messages, because it governs calls and the sidebar too, and making messages the authority on who belongs in a place would invert the dependency the repository keeps one-directional.
